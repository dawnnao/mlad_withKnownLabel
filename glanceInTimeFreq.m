function [sensorData, dateVec, dateSerial] = glanceInTimeFreq(pathRead, sensorNum, dayStart, dayEnd, pathSave, prefix, fs)
% DESCRIPTION:
...
for day = dayStart : dayEnd
    ...
    for hour = 0:23
        ...
        img = im2double(img);
        imshow(img)
        pathSaveAll = [pathSave '/' prefix num2str(count) '_time.png'];
...
%         close gcf
        ...
        freqData = fft(sensorData-median(sensorData));
        ...
        img = im2double(img);
        imshow(img)
        pathSaveAll = [pathSave '/' prefix num2str(count) '_freq.png'];
        imwrite(img, pathSaveAll);
%         close gcf
