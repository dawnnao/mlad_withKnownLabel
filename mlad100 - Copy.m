function sensor = mlad100(readRoot, saveRoot, sensorNum, dateStart, dateEnd, k, sensorTrainRatio, sensorPSize, fs, step, labelName)
% DESCRIPTION:
...
while goNext == 0
    ...
    for g = 1 : groupTotal
        for s = sensor.num{g}
            sensor.image{s} = sensor.image{s}(1:10000, :); % just time domain plot, no frequency domain
        end
    end
    ...
    [clust.data, clust.absIdx] = genDataForClust(sensor.numVec, hourTotal, sensorTrainRatio, sensor.image);
    % clustering
    fprintf('\nClustering...\n')
    clust.dataDownSamp = downsample(clust.data, 16); % dimension reduce of each sample
    clust.dataDownSamp = clust.dataDownSamp';
    clust.clustIdx = kmedoids(clust.dataDownSamp, k);
    clust.label = zeros(size(labelName, 2), size(clust.data, 2));  % label initialization
    fprintf('\nDone. Cluster overview:\n')
        ...
        for pBig = 1 : ceil(nIdxTemp/NP) % overview plot
            ...
            for pSmall = 1 : NP
                ...
                if count <= nIdxTemp
                   ...
                   imshow(reshape(clust.data(1:10000, idxTemp(count)), [100 100]));
                   subaxis(10,20, 2*pSmall, 'S',0.005, 'M',0.005);
                   imshow(ones(100, 100));
%                    imshow(reshape(clust.data(10001:20000, idxTemp(count)), [100 100]));
                else
                   subaxis(10,20, 2*pSmall-1, 'S',0.005, 'M',0.005);
                   imshow([]);
                   subaxis(10,20, 2*pSmall, 'S',0.005, 'M',0.005);
...
%             end
    ...
    for m = 1 : k
       trainSet.data{m} = clust.data(:, find(clust.clustIdx == m));
       trainSet.label{m} = clust.label(:, find(clust.clustIdx == m));
    end
    
    sTemp = trainSet.size; % samples to label
    cTemp = [1:k];
    ...
    while sTemp > 0
        sAver = ceil(sTemp/length(cTemp)); % average size  % division modification !!!!!!!!!!!
        for m = cTemp
            ...
            [trainSet.label, count] = dispAndLabelInTime(trainSet.data, trainSet.label, ...
                    m, trainSet.positionIn(m), trainSet.positionOut(m), count, trainSet.size, labelName);
        end
        sTemp = short; short = 0;
        cTemp = setdiff(cTemp, checkIn); checkIn = [];
    end
    close
    ...
    fprintf('\nTime consumption of training set making: %02dh%02dm%02ds\n\n\n\n', hours, mins, secs)
