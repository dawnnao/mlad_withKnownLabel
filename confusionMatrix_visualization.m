...
pathSave = 'D:\results\results_mlad_withKnownLabel\round3\images_in_confusion_matrix\';

path.root = 'C:/dataArchiveTemp/Sutong/'; % raw data path
fs = 20;

sequenceCube = 2; %: 49; %[19 31 20 38 21 45 26 32 27 39 28 46 34 40 35 47 42 48]; % [2 8 3 15 4 22 5 29 6 36 7 43 10 16 11 23 12 30 13 37 14 44 18 24 
labelTotal = size(label2012.sensor.label.name, 2);
for nCube = sequenceCube
    if  length(confTestInd{nCube}) > 1000
        numLimit = 1000;
    else
        numLimit = length(confTestInd{nCube});
    end
    
    for count = 1 : numLimit
        ...
        folderSave = sprintf('cube_%d/', nCube);
        if ~exist([pathSave '/' folderSave], 'dir')
            mkdir([pathSave '/' folderSave]);
        end
        
        fileSaveTemp = [pathSave '/' folderSave sprintf('cube_%d_numb_%d', nCube, count) '_TF.tif'];
        imwrite(imgTF, fileSaveTemp);
%         fileSaveTemp = [pathSave '/' folderSave sprintf('cube_%d_numb_%d', nCube, count) '_TF.emf'];
%         imwrite(imgTF, fileSaveTemp);

        fprintf('\nCube %d', nCube)
        fprintf('\nAll: %d, Now: %d\n', length(confTestInd{nCube}), count)
